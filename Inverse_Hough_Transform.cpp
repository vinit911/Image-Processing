...
/*We run a for loop throughout the sinusoid image and check for local maximas
using a kernel and a threshold for determining the maxima. 
The coordinates of each maxima are of the form (d,theta). 
Using the equation, d=x*cos(theta) + y*sin(theta), 
each maxima can be converted into a straight line and we get back the original binary image. 
This is basically the reverse process of converting a binary image to sinusoids in Hough space.*/
